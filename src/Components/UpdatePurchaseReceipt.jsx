...
function UpdatePurchaseReceipt() {
    ...
    const fetchReceipt = async(source, unMounted)=>{
        ...
        const receiptReq = baseURL.get(`purchaseReceipts/${receiptNumber}`, {
            cancelToken: source.token,
            headers:{
                'auth-token': user?.token
            }
        ...
        try {
            setLoader(true)
            const res = await axios.all([supReq, prodReq, receiptReq])
            const [supRes, prodRes, receiptRes] = await res
            ...
            const identifiedSup = suppliers?.filter(sup => sup._id === receipt?.supplier?._id && sup.id === receipt?.supplier?.id && sup.number === receipt?.supplier?.number)
            ...
            const supName = suppliers?.filter(sup => sup._id === receipt?.supplier?._id && sup.id === receipt?.supplier?.id && sup.number === receipt?.supplier?.number).map(sup => sup?.displayName)
    ...
    const submit = async()=>{
        ...
        try {
            setLoader(true)
            const {data} = await baseURL.put(`/purchaseReceipts/${receiptNumber}`, receiptData, {
                headers : {
                    'auth-token' : user?.token
                }
            })
        ...
        <div className="Quotations NewInvoice">
            ...
            <div className="formContainer">
                <form className='invoiceForm'>
                    ...
                    {
                        quoteInput?.supplier?.displayName && <div className="emailAndDetails">
                        <div className="customerEmail">
                            <p>Email: {quoteInput.supplier?.email}</p>
                        </div>
                        <div className="addressInfoCont">
                            <div className="deliveryAdress">
                                <h4>Billing Address</h4>
                                <div className="addressInfo">
                                    <p>Country: {quoteInput.supplier?.billingAddress?.country}</p>
                                    <p>City: {quoteInput.supplier?.billingAddress?.city}</p>
                                    <p>Street: {quoteInput.supplier?.billingAddress?.street}</p>
                                    <p>Tel: {quoteInput.supplier?.billingAddress?.tel}</p>
                                    <p>Mobile: {quoteInput.supplier?.billingAddress?.mobile}</p>
                                </div>
                            </div>
                            <div className="deliveryAdress">
                                <h4>Shiping Address</h4>
                                <div className="addressInfo">
                                    <p>Country: {quoteInput.supplier?.shippingAddress?.country}</p>
                                    <p>City: {quoteInput.supplier?.shippingAddress?.city}</p>
                                    <p>Street: {quoteInput.supplier?.shippingAddress?.street}</p>
                                    <p>Tel: {quoteInput.supplier?.shippingAddress?.tel}</p>
                                    <p>Mobile: {quoteInput.supplier?.shippingAddress?.mobile}</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default UpdatePurchaseReceipt
