...
function UpdateQuotation() {
    ...
    const sendInvoice = async()=>{
        await baseURL.post(`/invoices/sendInvoice/${invoiceData.input?.quoteNumber}-${user.userID}`, {cutomerDetails : quoteInput.customer}, {
            headers : {
                'auth-token' : user?.token
            }
    ...
    const submit = async()=>{
        ...
        await baseURL.put(`/quotations/quotes/${quote_id}`, invoiceData, {
            headers : {
                'auth-token' : user?.token
            }
        })
