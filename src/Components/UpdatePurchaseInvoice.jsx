...
function UpdatePurchaseInvoice() {
    ...
    const fetchInvoice = async(source, unMounted)=>{
        ...
        const ordersReq = baseURL.get('purchaseOrders', {
            cancelToken: source.token,
            headers:{
                'auth-token': user?.token
            }
        ...
        const invoiceReq = baseURL.get(`purchaseInvoices/${invoice_id}`, {
            cancelToken: source.token,
            headers:{
                'auth-token': user?.token
            }
        ...
        try {
            setLoader(true)
            const res = await axios.all([supReq, prodReq, invoiceReq, ordersReq])
            const [supRes, prodRes, invoiceRes, ordersRes] = await res
            ...
            const identifiedSup = suppliers?.filter(sup => sup._id === invoice?.supplier?._id && sup.id === invoice?.supplier?.id && sup.number === invoice?.supplier?.number)
            ...
            const supName = suppliers?.filter(sup => sup._id === invoice?.supplier?._id && sup.id === invoice?.supplier?.id && sup.number === invoice?.supplier?.number).map(sup => sup?.displayName)
            ...
            setQuoteInput(prev => ({
                ...
                orderRef : invoice?.input?.orderRef
            }))

            setOrderNum(invoice?.input?.orderRef?.number)
    ...
    const sendInvoice = async()=>{
        await baseURL.post(`/invoices/sendInvoice/${invoiceData.input.invoiceNumber}-${user.userID}`, {cutomerDetails : quoteInput.customer}, {
            headers : {
                'auth-token' : user?.token
            }
        }).then(res => {
    ...
    const submit = async()=>{
        ...
        try {
            setLoader(true)
            const {data} = await baseURL.put(`/purchaseInvoices/${invoice_id}`, invoiceData, {
                headers : {
                    'auth-token' : user?.token
                }
            })
    ...
    return (
        ...
        <div className="Quotations NewInvoice">
            <div className="addProductHeading">
                <h2>Update Purchase Invoice #{invoice?.input?.number}</h2>
                ...
                <form className='invoiceForm'>
                    <div className="invoiceFormTop">
                        ...
                        <div className="dueDate invoiceControl">
                            ...
                            <input type="text" id="dueDate" className='invoiceSelectInput' name="deliveryDate" value={quoteInput?.dueDate()} readOnly/>
                        </div>
                                ...
                                <div className="autoCompleteContainer">
                                    ...
                                    </button>
                                    {
                                        orders?.filter(item => {
                                            ...
                                            if (item.orderNumber?.toLowerCase().includes(orderNum?.toLowerCase())) {
                                            return true
                                            }
                                        })
                                        .map((item, i) => (
