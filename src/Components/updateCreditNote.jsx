...
function UpdateCreditNote() {
    ...
    const fetchCreditNote = async(source, unMounted)=>{
        ...
        const noteReq = baseURL.get(`creditNotes/${noteNumber}`, {
            cancelToken: source.token,
            headers:{
                'auth-token': user?.token
            }
        ...
        try {
            setLoader(true)
            const res = await axios.all([custReq, prodReq, noteReq])
            const [custRes, prodRes, noteRes] = await res
            const creditNote = await noteRes.data
            const customers = await custRes.data.customers
            

            const addedProds = await noteRes.data?.products

            const prods = await prodRes.data

            const identifiedCust = customers?.filter(cust => cust._id === creditNote?.customer?._id && cust.id === creditNote?.customer?.id && cust.number === creditNote?.customer?.number)
            ...
            const custName = customers?.filter(cust => cust._id === creditNote?.customer?._id && cust.id === creditNote?.customer?.id && cust.number === creditNote?.customer?.number).map(cust => cust?.displayName)

            setQuoteInput(prev => ({
                ...prev,
                date : creditNote.input.date,
                invoiceHeading : creditNote?.input?.heading,
                number : creditNote?.input?.number,
                customer: identifiedCust[0],
                invoiceRef : creditNote?.input?.invoiceRef
            }))

            setInvoiceRef(creditNote?.input?.invoiceRef?.number)

            setCustName(custName[0]);

            setAddedProducts(addedProducts)
            setAddedCharges(await noteRes.data.otherCharges)

            setCustomers(await customers)
            setProducts(await prodRes.data)
            setCreditNote(await creditNote)
    ...
    const invoiceData = {
        ...
        input : {
            ...quoteInput,
            noteNumber : creditNote?.input?.number,
            noteId : creditNote?.input?.id
        },
    ...
    return (
        ...
        <div className="Quotations NewInvoice">
            <div className="addProductHeading">
                <h2>Update Sales Return #{creditNote?.input?.number}</h2>
